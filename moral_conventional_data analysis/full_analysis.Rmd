...
author: "Rose M. Schneider, Miguel Mejia"
date: "`r Sys.Date()`"
output: pdf_document
---
...
rm(list = ls())
all_data = read.csv("data/cleaned_data/mc_all_data.csv")
study1_data <- read.csv("data/cleaned_data/study1_mc_all_data.csv")#study 1
study2_data <- read.csv("data/cleaned_data/study2_mc_iran.csv") #study 2
...
fig_1_v1 <- study1_data %>%
  ...
  scale_fill_manual(values = c(viridis::viridis(3), viridis::viridis(4))) +
  scale_color_manual(values = c(viridis::viridis(3), viridis::viridis(4)))

fig_1_v2 <- study1_data %>%
  filter(q_kind == 0)%>%
  dplyr::rename("Transgression" = "task")%>%
  mutate(Transgression = factor(Transgression, levels = c("moral", "conv"), 
                                labels = c("Moral", "Conventional")))%>%
  ggplot(aes(x = age, y= answer, color = Transgression, fill = Transgression)) + 
  geom_point(position = position_jitter(width = .0, height = .03), 
             alpha = .4) + 
  geom_smooth(method ="lm", fullrange = TRUE) +
  langcog::scale_color_solarized() + 
  langcog::scale_fill_solarized() + 
  ...
  # scale_x_continuous(expand = c(0,0), ) +
  # scale_x_continuous(breaks = seq(5, 11, 1), expand = c(0,0), limits = c(5, 11)) +
  theme(strip.text.x = element_text(size = 16),
        strip.text.y = element_text(size = 16, ),
        axis.text.x = element_text(angle = 0, size = 16, ),
        axis.text.y = element_text(size = 16),
...
fig_2_v1 <- study1_data %>%
  ...
  # langcog::scale_fill_solarized()+
  # scale_fill_hc("darkunica") +
  labs(y = 'Proportion "yes" Response', 
       x = "Site", 
       fill = "Transgression Type")  +
...
# fig2_table <- study1_data %>%
#   filter(q_kind != 0)%>%
#   mutate(task = factor(task, levels = c("moral", "conv"),
#                        labels = c("Moral", "Conventional")),
#          q_kind_label = factor(q_kind_label, levels = c("Everyone else",
#                                                         "Faraway country", 
#                                                         "Rule at school" ),
#                                labels = c("Would it be okay \nif everyone else were doing it?",
#                                           "Would it be okay \nin a faraway country?",
...
#   select()
  ...
  # group_by(CONDITION) %>%
  # summarise(participants = n()/12,
  #           critical_trials = n(),
  #           count_include = sum(Part, na.rm = T),
  #           prop_include = mean(Part, na.rm = T),
  #           sd_include = sd(Part, na.rm = T),
  #           se_include = sd_include/sqrt(critical_trials))
