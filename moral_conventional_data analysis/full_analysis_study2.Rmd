...
moral_conventional_figure_3 <- study2_data %>% 
  filter(q_kind == 0) %>%
...
### Visualization of permissibility by task
```{r}
moral_conventional_figure_4 <- study2_data %>%
  filter(q_kind != 0, 
         q_kind_label != "Haram", 
  ...
  mutate(task = factor(task, levels = c("moral", "conv", "conv.iran", "religious"), 
         ...
         q_kind_label = factor(q_kind_label, 
...
theme(legend.position = "none", 
  ...
  scale_y_continuous(breaks = c(0, 0.25, 0.5)) +
    theme(strip.text.x = element_text(size = 16),
        strip.text.y = element_text(size = 16, ),
        axis.text.x = element_text(size = 16, angle = 55, hjust = 1, vjust = 1),
        axis.text.y = element_text(size = 16),
        legend.text = element_text(size = 16),
        legend.title = element_text(size = 16),
        axis.title.x= element_text(vjust = -2.5, size=16),
        axis.title.y= element_text(vjust = 2.5, size=16),
        legend.position = "none",
        plot.margin=unit(c(6,6,6,6),"mm")) +
  theme(text=element_text(family = "serif")) +
...
```{r}
moral_conventional_figure_5 <- study2_data %>%
  filter(q_kind != 0, 
         q_kind_label == "Haram" | 
           q_kind_label == "Illegal" |
           q_kind_label == "Zesht")%>%
  ...
  mutate(q_kind_label = factor(q_kind_label, 
                               levels = c("A sin", "Illegal", "Rude"),
                               labels = c("Is it a sin?",
                                          "Is it illegal?",
                                          "Is it rude?"))) %>%
  ggplot(aes(x = task, y = answer, fill = task, color = task)) + 
  stat_summary(fun = mean, position = position_dodge(width = .9),
...
## MM: commented following two lines out 9/24/2024
  # langcog::scale_color_solarized() +
  # langcog::scale_fill_solarized() +
...
justifications <- read.csv("data/raw_data/study2/justifications_updated_6_27_2024.csv") %>%
...
my.cols.2 <- c("#b499bb", "#a6d3d1", "#ffe498", "#83a4bb")
...
moral_conventional_figure_6 <- justifications %>%
  filter(item != "")%>%
  mutate(transgression = ifelse(item %in% moral, "Moral", 
         ...
         transgression = factor(transgression, levels = c("Moral", "Conventional", 
                                                          "Conventional-Iran", "Religious")),
         Final.theme = str_to_title(Final.theme)) %>%
  ...
  theme(axis.text.y = element_text(margin = margin(0, 0, 0, 15))) +
  theme(panel.spacing = unit(1, "lines")) +
  scale_fill_manual(values = c(viridis::viridis(7))
                                  )

moral_conventional_figure_6
```
